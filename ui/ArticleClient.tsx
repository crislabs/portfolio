...
export function ArticleClient(props: Props) {
  const { article } = props
  const { data: post } = useQuery({
    queryKey: ['portfolio-get-article-client', article._id],
    queryFn: () => portfolioGetArticle(article._id),
    initialData: article,
  });
  ...
  return (
    <article className="max-w-2xl px-6 py-24 mx-auto space-y-12 dark:bg-gray-800 dark:text-gray-50">
      <div className="w-full mx-auto space-y-4 text-center">
        ...
        <h1 className="text-4xl font-bold leading-tight md:text-5xl">{post.dataArticle.seoArticle.title}</h1>
      ...
      <div className="text-gray-800 prose max-w-none prose-pre:p-0 prose-pre:bg-inherit">
          <MarkdownPreview markdown={post.dataArticle.content || ''}></MarkdownPreview>
          {/* <Markdown>
